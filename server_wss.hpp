...
namespace SimpleWeb {
    ...
    public:
        SocketServer(unsigned short port, size_t num_threads, const std::string& cert_file, const std::string& private_key_file, 
                ...
                const std::string& verify_file=std::string(), std::string document_root=std::string()) :
                SocketServerBase<WSS>::SocketServerBase(port, num_threads, timeout_request, timeout_idle, document_root),
